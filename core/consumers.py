...
class NewsConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        # Send message to room group
        await self.channel_layer.group_send(
            self.room_group_name, {"type": "news_fields", "news": news,"author": author, "content": content, "comment": comment}
        )
        # print(f'Received message from {self.channel_name}: {message}')
    ...
    async def news_fields(self, event):
        ...
        comment = event["comment"]
        # Send message to WebSocket
        await self.send(text_data=json.dumps({"news": news, "author": author, "content": content, 'comment': comment}))
